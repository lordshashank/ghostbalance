...
import {
  ...
  PUBLIC_BALANCE_ETH, NULLIFIER_SEED_ETH, BLOCK_NUMBER,
...
async function main() {
  ...
  const nullifierSeedWei = ethToWei(NULLIFIER_SEED_ETH);
  ...
  // nullifier_seed is unconstrained by the circuit (no <= account_balance assert)
  // so we allow any value here — it only affects nullifier derivation
  ...
  const headerRlpPadded = rightPadArray(proofData.header.rlpBytes, 741);
  ...
  // Compute link_in for B2: commit(stateRoot, addressHash, keyPtr=0, nullifierSeed, blinding)
  const linkB2In = await computeLinkJS(stateRoot, addressHashBytes, 0, nullifierSeedWei, BigInt(BLINDING));
  ...
  const linkB3In = await computeLinkJS(currHashB3, addressHashBytes, afterB2.keyPtr, nullifierSeedWei, BigInt(BLINDING));
  ...
  const linkB4In = await computeLinkJS(currHashB4, addressHashBytes, afterB3.keyPtr, nullifierSeedWei, BigInt(BLINDING));
  ...
  log('    - Blinded Poseidon2 commitment (address + nullifier_seed)');
...
// ---------------------------------------------------------------------------
// Compute Poseidon2 link commitment in JavaScript (via bb.js)
// ---------------------------------------------------------------------------
...
// Convert a BigInt to a 32-byte big-endian Uint8Array (Fr representation for bb.js)
function bigintToFr(val) {
...
// Poseidon2 hash via bb.js (same sponge construction used by Noir's poseidon2 crate)
let bbApi = null;
...
async function initPoseidon2() {
  ...
  try {
    ...
    bbApi = await BarretenbergSync.new();
  } catch (e) {
    logError('@aztec/bb.js not available. Install it: npm install @aztec/bb.js');
...
function poseidon2Hash2(a, b) {
  const result = bbApi.poseidon2Hash({ inputs: [bigintToFr(a), bigintToFr(b)] });
  return frToBigint(result.hash);
}

async function computeLinkJS(currHash, addressHash, keyPtr, nullifierSeed, blinding) {
  await initPoseidon2();
  ...
  const h1 = poseidon2Hash2(chHi, chLo);
  const h2 = poseidon2Hash2(ahHi, ahLo);
  const h3 = poseidon2Hash2(BigInt(keyPtr), nullifierSeed);
  const left = poseidon2Hash2(h1, h2);
  const right = poseidon2Hash2(h3, blinding);
  const result = poseidon2Hash2(left, right);
